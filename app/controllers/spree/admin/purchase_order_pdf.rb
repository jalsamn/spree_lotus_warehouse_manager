...
class Spree::PurchaseOrderPdf < Prawn::Document
  def initialize(purchase_order_items)
    super()
  ...
  def product_rows
    [['ID', 'Quantity', 'QuantityType']] +
      @purchase_order_items.map do |purchase_order_item|
        [purchase_order_item.id, purchase_order_item.quantity, purchase_order_item.quantitytype]
    end
  end
end